...
def add_args():
    ...
    parser.add_argument(
        "--exp_path",
        "-wp",
        help="project working path",
        type=str,
        required=True,
    )

    parser.add_argument('--time_stamp', type=str, default=datetime.now().strftime('%Y-%m-%d_%H:%M:%S.%f'))
    ...
    parser.add_argument("--worker_hostname", type=str, default=socket.gethostname())
...
args = Arguments(add_args())

datasetCategories = {'mnist': 10, 'cifar10': 10, }

args.num_class = datasetCategories[args.dataset] if args.dataset in datasetCategories else 10
# def load_arguments():
#     cmd_args = add_args()
#     # Load all arguments from YAML config file
